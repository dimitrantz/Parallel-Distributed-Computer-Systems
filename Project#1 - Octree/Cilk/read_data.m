function [results] = read_data(src, nL, nP, nN, nT, iter)
% function [results] = read_data(src, nL, nP, nN,iter)
% 
% input: 
%   src: The file 3containing the raw results, produced by the run_tests.sh script 
...
hash_mean = reshape(hash_mean, nL, nP, nN, nT);
...
morton_mean = reshape(morton_mean, nL, nP, nN, nT);
...
sort_mean = reshape(sort_mean, nL, nP, nN, nT);
...
dataR_mean = reshape(dataR_mean, nL, nP, nN, nT);

...
int main(int argc, char** argv){
  ...
  if (argc != 7) { // Check if the command line arguments are correct 
    printf("Usage: %s N dist pop rep L\n"
	   "where\n"
	   "N    : number of points\n"
	   ...
	   "CILK_NWORKERS : number of workers.\n", argv[0]);
    return (1); 
  }
 ...
 __cilkrts_set_param("nworkers","argv[6]"); // Set the number of workers to be used
  ...
  unsigned int *level_record = (unsigned int *) calloc(N,sizeof(unsigned int)); 
  ...
  for(int it = 0; it<repeat; it++){						 
    ...
    // Check if all particles that are in the same box have the same encoding. 
